...
def hello():
    if request.method == "POST":
        word_selected = request.form.get('word')
        ...
        print(word_selected)#Server is getting selections from front end
        print(language_selected)#Server is getting selections from front end
        print(part_of_speech_selected)#Server is getting selections from front end
        print(part_of_speech_ID)#Server is retrieving part of speech ID
        LineID_List = logic.SQLQuery(f"select Line_Text from {language_selected}_corpus where Line_Text like  '%{word_selected}%';")
        print(LineID_List)
